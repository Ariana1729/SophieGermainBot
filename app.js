var TeleBot = require("telebot"),
    bot = new TeleBot(process.env.token),
...
MongoClient.connect(url, function(err, client) {
    ...
    var db = client.db("sophiegermainbotdb");
    var questions = db.collection("questions");
    questions.deleteMany({}, function(err, res) {
        if (err) {
            return console.log(err);
        }
    });
    var previd = 0,
        topiclist = ["Algebra", "Calculus", "Geometry & Topology", "Number theory", "Others", "Physics"],
        groupid = -1001062964615,
        topictxt = "";
    ...
    function sendtmpmsg(fmsg, msg) {
        return bot.sendMessage(fmsg.chat.id, msg, {
            replyToMessage: fmsg.message_id,
            parseMode: "markdown",
            notification: false
        }).then(function(smsg) {
    ...
    bot.on("/add", function(msg) {
        ...
        for (let i = 0; i < topiclist.length + 1; ++i) {
            if (qn.endsWith(" " + topiclist[i])) {
                let qnname = qn.substring(0, qn.length - topiclist[i].length - 1),
                    qntopic = topiclist[i];
                ...
                questions.find({
                    ...
                    questions.insert([{
                        ...
                        return msg.reply.text("Added question", {
                            asReply: true
                        });
                    });
                });
        ...
        for (let i = 0; i < topiclist.length; ++i) {
            if (qn.endsWith(" " + topiclist[i])) {
                ...
                questions.find({
                    ...
                    if (!isadmin && qn[0].op != msg.from.id) {
        ...
        for (let i = 0; i < topiclist.length; ++i) {
            if (qn.endsWith(" " + topiclist[i])) {
                ...
                questions.find({
                    ...
                    return bot.sendMessage(msg.chat.id, "Here is the question", {
                        replyToMessage: qn[0].msgid
                    });
                });
            }
        }
